...
test('Filter queries should accept a list of tag values', t => {
    const query = {
        tags: ['simple:valuea', 'simple:valueb'],
    };
    const { value } = querySchema.validate(query);
    t.deepEqual(value, { tags: ['simple:valuea', 'simple:valueb'] });
});
...
test('Filter queries should reject tag values with missing type prefix', t => {
    ...
    t.deepEqual(
        error.details[0].message,
        '"tags[0]" with value "simple" fails to match the tag pattern',
    );
});
...
test('Filter queries should allow project names', t => {
    ...
    const { value } = querySchema.validate(query);
    t.deepEqual(value, { projects: ['projecta'] });
});

test('Filter queries should reject project names that are not alphanum', t => {
    const query = {
        projects: ['project name with space'],
    };
    const { error } = querySchema.validate(query);
    t.deepEqual(
        error.details[0].message,
        '"projects[0]" must only contain alpha-numeric characters',
    );
});
