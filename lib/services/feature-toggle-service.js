...
class FeatureToggleService {
    ...
    async getFeatures({ tags, projects }) {
        if (!tags && !projects) {
            return this.featureToggleStore.getFeatures();
        }
        const query = await querySchema.validateAsync({ tags, projects });
        query.tags = query.tags.map(q => q.split(':'));
        return this.featureToggleStore.filterFeatures(query);
    }
