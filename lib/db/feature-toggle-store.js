...
const FEATURE_TAG_COLUMNS = ['tag_type', 'tag_value'];
...
class FeatureToggleStore {
    ...
    async filterFeatures(query) {
        const stopTimer = this.timer('getAllFiltered');
        ...
        if (query.tags) {
            const tagQuery = this.db
                .from('feature_tag')
                .select('feature_name')
                .whereIn(FEATURE_TAG_COLUMNS, query.tags);
            baseQuery = baseQuery.whereIn('name', tagQuery);
        }
        if (query.projects) {
            baseQuery = baseQuery.whereIn('project', query.projects);
        }
